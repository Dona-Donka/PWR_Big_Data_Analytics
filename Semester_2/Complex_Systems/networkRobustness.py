...
def computeComponent(size):
    ...
    for r in range(1,5):
        print("step: ", r)
        graph = nx.random_geometric_graph(10 ** size, 0.5)
        graph1 = graph.copy()
        graph2 = graph.copy()
        graph3 = graph.copy()
    ...
    print("x: ", x)
    degree, = plt.plot(x, degreeCompMean, label = "degree (attack)")
    randoms, = plt.plot(x, randomCompMean, label = "random")
    closeness, =   plt.plot(x, closenessCompMean, label = "closeness")
    betweeness = plt.plot(x, betweennessCompMean, label = "betweeness")

    # drawing legend and titles:
    legend = plt.legend(bbox_to_anchor=(0.96, 0.94), loc = "upper right", borderaxespad=0.)
    plt.gca().add_artist(legend)
    plt.title("Robustness of networks" + "\n" + "Random Graph, N = 1000, k = 8")
    plt.xlabel("Removed nodes")
    plt.ylabel("connected components coefficient")
    plt.savefig("random_N1000_k8.jpg")
